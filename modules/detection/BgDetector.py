...
class BgDetector:
    ...
    def loadSettings(self, path, video_settings_file):
        ...
        self.maxSkeletonSize = c.getint("max_skeleton_length")  # minimum length between two keypoint in the skeleton (cam1), for the distance to be considered when finding best keypoint
        ...
        if self.region_name not in region_info.keys():
            print(f"{self.region_name} not in")
            exit(0)
...
def generate_SingleBgDetect(region_name):
    ...
    bgDet_file = os.path.join(bgDet_out_path, video_name.replace("mp4", "csv").replace("avi", "csv").replace("MOV", "csv"))
...
if __name__ == '__main__':
    ...
    if (platform.system() == 'Windows'):
        ap.add_argument("-f", "--path", default='E:\\data\\3D_pre\\D1_T3', help="Path to folder")
    ...
    ap.add_argument("--video_name", default='2021_10_11_22_38_00_ch08.avi', help="camerid and recording datetime")
    ...
    if not cap.isOpened():
        ...
        sys.exit()
    if not os.path.isfile(bg_file):
        print(f"No background image present：{bg_file}")
        print("... creating one.")
        # background的参数属于基础设置，在path目录下
        if not os.path.exists(bg_file):
            os.makedirs(os.path.dirname(bg_file))
