...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    if not os.path.exists(project):
        os.makedirs(project)
    txt_path = str(f"{project}/{Video_name[:-4]+'.csv'}")
    f = open(txt_path, 'w+')
    f.write(
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='E:\\code/3D-ZeF/modules/Sessions/train_ZeF_top/zerbrafish_top3/weights/best.pt',
                        help='model.pt path(s)'
                        )
    parser.add_argument('--config_path', type=str,
                        # default='/home/data/HJZ/zef/exp_pre/',
                        default='E:\\data\\OCU_ZeF',
                        help='config path'
                        )
    parser.add_argument('--region_name', type=str, default='6PPD1ppm', help='config path')

    parser.add_argument('--source', type=str,
                        default='E:\\data\\OCU_ZeF\\cut_video/20211027_ch12.MOV',
                        help='file/dir/URL/glob, 0 for webcam'
                        )
    parser.add_argument('--project',
                        default='E:\\data\\OCU_ZeF\\processed\\1_Mutant',
                        help='save results to project/name'
                        )

    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[540,960], help='inference size h,w')
