...
sys.path.append('../')
sys.path.append('../../')
from common.utility import csv2Tracks, readConfig, getTrackletFeatures, frameConsistencyGraph, \
    getDropIndecies
...
class TrackFinalizer:
    ...
    def __init__(self, dataPath, cfgPath, bboxResize=None):
        """
        Initialize object
        ...
        self.loadSettings(cfgPath, bboxResize)
        self.path = dataPath
    ...
    def getMetrics(self, tDistm, sDistm, iFramesm, iFramesRatiom, iTDistm, iSDistm, verbose=False):
        ...
        if verbose:
            print(
                "Valid metrics\n\tTemporal: {}\n\tSpatial: {}\n\tIntersection: {}\n\tIntersection Ratio: {}\n\tInternal Temporal: {}\n\tInternal Spatial: {}\n".format(
                    temporalData, spatialData, intersectionData, intersectionRatioData, internalTemporalData,
                    internalSpatialData))
    ...
    def frameTo3DCoord(self, track, frameIdx):
        ...
        return np.array([track.x[frameIdx], track.y[frameIdx]])
    ...
    def rankTrackletTemporally(self, galleryTracks, mainTracks, verbose=True):
        ...
        if verbose:
            for mTrack in mainTracks:
                mt = self.mainTracks[mTrack]
                print("Main track {} - length {} - detections {} - consistency {} - start {} - end {}".format(mTrack,
                                                                                                              mt.frame[
                                                                                                                  -1] -
                                                                                                              mt.frame[
                                                                                                                  0] + 1,
            ...
            for lst in tempDistList:
                print("Track {} - temporal Distance {} - length {} - start {} - end {}".format(lst[0], lst[1], lst[2],
                                                                                               self.tracks[
                                                                                                   lst[0]].frame[0],
                                                                                               self.tracks[
                                                                                                   lst[0]].frame[-1]))
    ...
    def connectTracklets(self, galleryTracks, mainTracks):
        ...
        for mTrack in mainTracks:
            ...
            print("\tLength {} - Detections {} - Consistency {} - Start {} - End {}".format(
                mt.frame[-1] - mt.frame[0] + 1, len(mt.frame), len(mt.frame) / (mt.frame[-1] - mt.frame[0] + 1),
                mt.frame[0], mt.frame[-1]))
            if len(gap) > 0:
                print(
                    "\tLargest Gap {} - Mean Gap {} - Median Gap {} - Std Dev Gap {} - Max Gap {} - Min Gap {} - # Gaps {}".format(
                        np.max(gap), np.mean(gap), np.median(gap), np.std(gap), np.max(gap), np.min(gap), len(gap)))
            if len(sDist) > 0:
                print(
                    "\tLargest Dist {} - Mean Dist {} - Median Dist {} - Std Dev Dist {} - Max Dist {} - Min Dist {}\n".format(
                        np.max(sDist), np.mean(sDist), np.median(sDist), np.std(sDist), np.max(sDist), np.min(sDist)))

        # if not os.path.exists(os.path.join(self.path, "processed")):
        #     os.makedirs(os.path.join(self.path, "processed"))
        ...
        if notAssigned:
            ...
            print(
                "Largest unassigned tracklet: ID {} - Mean {} - Median {} - Std Dev {} - Min {} - Max {} - # {}".format(
                    na_ids[np.argmax(na_len)], np.mean(na_len), np.median(na_len), np.std(na_len), np.min(na_len),
                    np.max(na_len), len(na_len)))
    ...
    def findConccurent(self, trackIds, overlaping=True, fully_concurrent=False, verbose=True):
        ...
        for idx1 in range(len(trackIds)):
            ...
            if verbose:
                print("Track1: {}, fStart {},  fEnd {}, # frames {}, # missing frames {}".format(track1Id,
                                                                                                 tracks[track1Id].frame[
                                                                                                     0],
                                                                                                 tracks[track1Id].frame[
                                                                                                     -1], len(
                        tracks[track1Id].frame), (tracks[track1Id].frame[-1] - tracks[track1Id].frame[0] + 1) - len(
                        tracks[track1Id].frame)))
            ...
            for idx2 in range(idx1 + 1, len(trackIds)):
                ...
                if verbose:
                    print("Track2: {}, fStart {},  fEnd {}, # frames {}, # missing frames {}".format(track2Id, tracks[
                        track2Id].frame[0], tracks[track2Id].frame[-1], len(tracks[track2Id].frame), (tracks[
                                                                                                          track2Id].frame[
                                                                                                          -1] - tracks[
                                                                                                          track2Id].frame[
                                                                                                          0] + 1) - len(
                        tracks[track2Id].frame)))
    ...
    def findMainTracks(self):
        ...
        All tracks are then checked for overlap, and all valid cases where N tracks overlap eachother more than a user defined value,
        are then returned.
        ...
        for tr in self.tracks:
            ...
            length.append(
                len(np.linspace(track.frame[0], track.frame[-1], track.frame[-1] - track.frame[0] + 1, dtype=np.int)))

            print("Track {}: Start {}, End {}, Length {}, Detections {}".format(tr, track.frame[0], track.frame[-1],
                                                                                length[-1], detections[-1]))
                ...
                for idx2 in range(idx1, self.n_fish):
                    for idx3 in range(self.n_fish):
                        if paths[idx1][idx3] != paths[idx2][idx3]:
                            print("ERROR {}-{}-{} / {}-{}-{}".format(idx1, idx3, paths[idx1][idx3], idx2, idx3,
                                                                     paths[idx2][idx3]))
...
def updateIds(df, paths, verbose=True):
    ...
    df[['id_str', 'cam_str']] = df[['id', 'cam']].astype(str)
    df['id'] = 'id'+df['id_str']+"_cam"+df['cam_str']
...
def cal_3std(df_data, col_name):
    mean = df_data[col_name].mean()
    std = df_data[col_name].std()
    minvalue = mean - 10 * std
    maxvalue = mean + 10 * std
    # iqr = df_data[col_name].quantile(0.75) - df_data[col_name].quantile(0.25)
    # # 根据iqr计算异常值判断阈值
    # maxvalue = df_data[col_name].quantile(0.75) + 1.5 * iqr  # 上界
    # minvalue = df_data[col_name].quantile(0.25) - 1.5 * iqr  # 下界


    return minvalue, maxvalue

def dropIrregular(df):
    # drop 过大的检测框
    df['area'] = df['aa_w'] * df['aa_h']
    big_ind = df[df['area'] > 4000].index
    if len(big_ind) > 0:
        print(f"length of bigger box is {len(big_ind)}")
        df.drop(index=big_ind, inplace=True)

    sm_ind = df[df['area'] < 100].index
    if len(sm_ind) > 0:
        print(f"length of smaller box is {len(sm_ind)}")
        df.drop(index=sm_ind, inplace=True)
    # drop 不合理位移
    dist_data = df[['frame', 'c_x', 'c_y']].diff(1)
    dist_data['dist'] = (dist_data['c_x'] ** 2 + dist_data['c_y'] ** 2)**(1/2)

    drop_condition = (dist_data['dist'] >= 200) & (dist_data['dist'] == 1)
    drop_ind = df[drop_condition].index
    print(f"drop larger dist 250: {len(drop_ind)}")
...
def interpolate(df):
    ...
    for ikey in dummyRow.keys():
        dummyRow[ikey] = np.nan
    ...
    for i in trackIds:
        ...
        for columnHeader in currTrack.columns:
            if columnHeader not in ['time_str', 'cam', 'id_str', 'cam_str']:
...
if __name__ == '__main__':
    ...
    ap.add_argument("--RegionName", default='4_6PPDQ500ppb', help="region name of experience")
    ap.add_argument("--tracker", default='sortTracker', help="region name of experience")
    ap.add_argument("--track_file", default='2021_10_15_07_49_59_ch12.csv', help="region name of experience")
    ...
    video_file = track_file.replace("csv", "avi")

    camNO = track_file.split(".")[0].split("_")[-1]
    video_nameT = '_'.join(track_file.split(".")[0].split("_")[: -1])

    path = os.path.join(root_path, DayTank)
    # Check if /processed/ folder exists
    track_folder = os.path.join(path, tracker, region_name)
    if not os.path.isdir(track_folder):
        os.makedirs(track_folder)

    dataPath = track_folder

    # Load tracklets and CSV file
    tracks = csv2Tracks(os.path.join(dataPath, track_file), offset=0, minLen=15)
    oldCsv = pd.read_csv(os.path.join(dataPath, track_file), index_col=0)

    tf = TrackFinalizer(dataPath, cfgPath=path)
    ...
    if (len(tracks) > tf.n_fish):
        ...
        print("Only {} 3D tracklets are present, while there are {} fish. Not enough informaiton to work with".format(
            len(tracks), tf.n_fish))
        sys.exit()

    linkedCsv.dropna(inplace=True)
    # Remove duplicate frame detections in a track
    linkedCsv.reset_index(inplace=True, drop=True)
    drop_ind = getDropIndecies(linkedCsv)
    print(f"Remove duplicate frame detections in a track {len(drop_ind)}")
    linkedCsv = linkedCsv.drop(drop_ind)

    # Check if /processed/ folder exists
    out_folder = os.path.join(path, 'mergeId', region_name)
    if not os.path.isdir(out_folder):
        os.makedirs(out_folder)
    outputPath = os.path.join(out_folder, track_file)

    print("Saving merge data to: {0}".format(outputPath))
    linkedCsv.to_csv(outputPath, index=False)

    # remove small detection box
    regular_data = dropIrregular(linkedCsv)

    # Interpolate missing detections in linked csv file
    interpolated = interpolate(regular_data)
    ...
    # Check if /processed/ folder exists
    out_folder = os.path.join(path, 'finalTrack', region_name)
    if not os.path.isdir(out_folder):
        os.makedirs(out_folder)
    outputPath = os.path.join(out_folder, track_file)
    print("Saving interpolation data to: {0}".format(outputPath))
